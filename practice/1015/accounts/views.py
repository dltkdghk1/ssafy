...
def profile(request, username):
  # get_user_model() : 프로젝트에 활성화 되어 있는 모든 사용자 모델(로그인한 사용자 모델 아님!)
  User = get_user_model()
  # person : 게시글 작성한 개인 사용자, 반드시 로그인한 사용자일 필요는없음
...
def follow(request, user_pk):
  User = get_user_model()
  # person : user_pk로 팔로우하려는 상대방
  person = User.objects.get(pk=user_pk)

  if person != request.user:
    # 로그인한 사용자가 이미 팔로잉을 하고 있는지 확인
    # filter.exists() : db에 존재하는지 여부 확인 -> True or False가 나옴
    ...
    # 팔로우/언팔로우 후 DB변경 -> 페이지 랜더링 X / 리다이렉트
